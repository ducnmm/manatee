...
test('buildMentionQuery windows search like dugong (since_time/until_time)', () => {
  assert.equal(
    ...
    '(@ManateeWallet OR from:ManateeWallet OR to:ManateeWallet) (send OR register) since_time:1700000000 until_time:1700000030',
  );
  assert.equal(
    buildMentionQuery('@ManateeWallet', 1, 2),
    '(@ManateeWallet OR from:ManateeWallet OR to:ManateeWallet) (send OR register) since_time:1 until_time:2',
  );
});
